...
export function squareToWorld(square: SquareId, y = BOARD_TOP): THREE.Vector3 {
  ...
  return new THREE.Vector3((file - 4) * TILE, y, (4.5 - rank) * TILE);
}

export function worldToSquare(x: number, z: number): SquareId | null {
  const file = Math.round(x / TILE + 4);
  const rank = Math.round(4.5 - z / TILE);
  if (file < 0 || file >= FILES.length || rank < 0 || rank >= RANKS) return null;
...
/**
 * The playing surface: 90 bevelled Xiangqi intersections on a carved base with a
...
export class BoardView {
  ...
  constructor() {
    ...
    for (let rank = 0; rank < RANKS; rank += 1) {
      for (let fileIndex = 0; fileIndex < FILES.length; fileIndex += 1) {
        ...
        const tile = new THREE.Mesh(tileGeometry, lightMaterial);
  ...
  private buildBase(): void {
    const width = TILE * FILES.length + 1.5;
    const depth = TILE * RANKS + 1.5;
    const geometry = this.track(new RoundedBoxGeometry(width, 0.62, depth, 4, 0.09));
    ...
    const trimGeometry = this.track(new RoundedBoxGeometry(width + 0.18, 0.14, depth + 0.18, 3, 0.06));
  ...
  /** River and palace diagonals make the expanded 9×10 field read as Xiangqi. */
  private buildXiangqiMarks(): void {
    const riverMaterial = this.track(
      new THREE.MeshBasicMaterial({ color: 0x173c4a, transparent: true, opacity: 0.38, depthWrite: false }),
    );
    const river = new THREE.Mesh(new THREE.PlaneGeometry(TILE * 8.9, TILE * 0.78), riverMaterial);
    this.track(river.geometry);
    river.rotation.x = -Math.PI / 2;
    river.position.set(0, BOARD_TOP + 0.006, 0);
    river.renderOrder = 1;
   ...
   * read instantly instead of competing with 90 evenly-lit intersections.
   */
