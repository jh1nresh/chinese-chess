...
/** Warm rice-paper window crossed by a simple Chinese lattice. */
function latticeTexture(): THREE.CanvasTexture {
  ...
  const glow = ctx.createLinearGradient(0, 0, 256, 512);
  glow.addColorStop(0, "#f6d79d");
  glow.addColorStop(0.5, "#fff0c6");
  glow.addColorStop(1, "#b95f2f");
  ctx.fillStyle = glow;
  ctx.fillRect(0, 0, 256, 512);

  ctx.strokeStyle = "#5d1714";
  ctx.lineWidth = 15;
...
/**
 * A ruined Chinese command courtyard: flagstone floor, a ring of vermilion
 * columns and dark eaves, rice-paper lattice windows, drifting dust and four
 * glowing palace lanterns around the board.
 */
export class CastleHall {
  ...
  private buildColonnade(): void {
    ...
    const beamGeo = this.track(new THREE.BoxGeometry(4.5, 0.44, 0.62));
    const eaveGeo = this.track(new THREE.BoxGeometry(5.1, 0.2, 1.5));
    const eaveMat = this.track(
      new THREE.MeshStandardMaterial({ color: 0x171b19, roughness: 0.72, metalness: 0.18 }),
    );
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      // Dark timber beam and a broad upturned-looking eave span each bay.
      const next = ((i + 1) / count) * Math.PI * 2;
      const mid = (angle + next) / 2;
      const beam = new THREE.Mesh(beamGeo, stone);
      beam.position.set(Math.cos(mid) * radius, 9.55, Math.sin(mid) * radius);
      beam.rotation.y = -mid + Math.PI / 2;
      beam.castShadow = true;
      this.group.add(beam);

      const eave = new THREE.Mesh(eaveGeo, eaveMat);
      eave.position.set(Math.cos(mid) * radius, 10.05, Math.sin(mid) * radius);
      eave.rotation.y = -mid + Math.PI / 2;
      eave.rotation.z = i % 2 === 0 ? 0.035 : -0.035;
  ...
  private buildTorches(): void {
    const bracketMat = this.track(new THREE.MeshStandardMaterial({ color: 0x211712, roughness: 0.68, metalness: 0.42 }));
    const lacquerMat = this.track(
      new THREE.MeshStandardMaterial({ color: 0x7b1816, roughness: 0.5, metalness: 0.08 }),
    );
    const goldMat = this.track(
      new THREE.MeshStandardMaterial({ color: 0xc5963d, roughness: 0.38, metalness: 0.72 }),
    );
    const stemGeo = this.track(new THREE.CylinderGeometry(0.1, 0.14, 3.4, 8));
    const armGeo = this.track(new THREE.BoxGeometry(1.25, 0.1, 0.12));
    const lanternGeo = this.track(new THREE.SphereGeometry(0.52, 16, 12));
    const ringGeo = this.track(new THREE.TorusGeometry(0.36, 0.045, 8, 18));
    const tasselGeo = this.track(new THREE.CylinderGeometry(0.025, 0.05, 0.72, 6));
    ...
    positions.forEach(([x, z], index) => {
      ...
      arm.position.set(x, 2.82, z);
      arm.rotation.y = index % 2 === 0 ? Math.PI / 4 : -Math.PI / 4;
      this.group.add(arm);

      const shell = new THREE.Mesh(lanternGeo, lacquerMat);
      shell.position.set(x, 3.38, z);
      shell.scale.set(0.84, 1.18, 0.84);
      shell.castShadow = true;
      this.group.add(shell);

      for (const y of [2.91, 3.85]) {
      ...
      const tassel = new THREE.Mesh(tasselGeo, lacquerMat);
      tassel.position.set(x, 2.48, z);
      this.group.add(tassel);

      const flameMat = this.track(
        new THREE.MeshBasicMaterial({ color: 0xffc56d, transparent: true, opacity: 0.78 }),
      );
      this.flameMaterials.push(flameMat);
      const flame = new THREE.Mesh(this.track(new THREE.SphereGeometry(0.36, 12, 10)), flameMat);
      flame.position.set(x, 3.38, z);
      flame.scale.set(0.8, 1.25, 0.8);
      this.group.add(flame);

      const light = new THREE.PointLight(0xffa044, 22, 22, 2);
      light.position.set(x, 3.4, z);
      this.group.add(light);

      this.torches.push({ light, flame, seed: index * 12.7, base: 22 });
    });
  }
  ...
  applyArena(look: ArenaLook): void {
    ...
    this.pillarMaterial?.color.setHex(look.id === "frost" ? 0x7b2925 : 0x651714);
