...
export class JungleOverlay {
  ...
  /** A distant Chinese mountain temple: stone terrace, red halls and stacked golden eaves. */
  private buildTemple(angle: number, radius: number, scale: number, gilded: boolean): THREE.Group {
    ...
    const lacquer = this.templeMoss as THREE.MeshStandardMaterial;
    const gold = this.templeGold as THREE.MeshStandardMaterial;

    const terraceGeo = this.track(new THREE.BoxGeometry(28, 3.2, 22));
    const terrace = new THREE.Mesh(terraceGeo, stone);
    terrace.position.y = 1.6;
    temple.add(terrace);

    const stairGeo = this.track(new THREE.BoxGeometry(7, 1.2, 8));
    for (let i = 0; i < 6; i += 1) {
      const stair = new THREE.Mesh(stairGeo, stone);
      stair.position.set(0, 0.55 + i * 0.52, 14.2 - i * 1.1);
      stair.scale.set(1, 1, 1 - i * 0.07);
      temple.add(stair);
    }

    let floorY = 3.2;
    for (let tier = 0; tier < 3; tier += 1) {
      const width = 19 - tier * 4.2;
      const depth = 14 - tier * 3;
      const hallHeight = 5.2 - tier * 0.5;
      const hallGeo = this.track(new THREE.BoxGeometry(width * 0.82, hallHeight, depth * 0.72));
      const hall = new THREE.Mesh(hallGeo, lacquer);
      hall.position.y = floorY + hallHeight / 2;
      temple.add(hall);

      const columnGeo = this.track(new THREE.CylinderGeometry(0.34, 0.42, hallHeight + 0.3, 10));
      for (const x of [-width * 0.34, 0, width * 0.34]) {
        for (const z of [-depth * 0.31, depth * 0.31]) {
          const column = new THREE.Mesh(columnGeo, lacquer);
          column.position.set(x, floorY + hallHeight / 2, z);
          temple.add(column);
        }
      }

      const roofGeo = this.track(new THREE.ConeGeometry(width * 0.72, 3.1, 4));
      const roof = new THREE.Mesh(roofGeo, gilded && tier === 2 ? gold : stone);
      roof.position.y = floorY + hallHeight + 1.25;
      roof.rotation.y = Math.PI / 4;
      roof.scale.z = depth / width;
      temple.add(roof);

      const ridgeGeo = this.track(new THREE.CylinderGeometry(0.18, 0.18, width * 0.92, 8));
      const ridge = new THREE.Mesh(ridgeGeo, gold);
      ridge.position.y = floorY + hallHeight + 2.78;
      ridge.rotation.z = Math.PI / 2;
      temple.add(ridge);

      floorY += hallHeight + 2.7;
    }

    if (gilded) {
      const discGeo = this.track(new THREE.CircleGeometry(1.8, 24));
      const discMat = this.track(new THREE.MeshBasicMaterial({ color: 0xffd06a, fog: false }));
      this.sunDiscMaterial = discMat;
      const disc = new THREE.Mesh(discGeo, discMat);
      disc.position.set(0, 8.1, 5.35);
      temple.add(disc);
    }
