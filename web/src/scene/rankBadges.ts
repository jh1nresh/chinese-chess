...
function drawPieceCharacter(
  ctx: CanvasRenderingContext2D,
  kind: PieceKind,
  faction: Faction,
  size: number,
): void {
  ctx.font = `700 ${size}px KaiTi, STKaiti, DFKai-SB, serif`;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText(PIECE_CHARACTERS[faction][kind], 50, 52);
}
...
const THEMES: Record<Faction, BadgeTheme> = {
  w: {
    rim: "#f1c77a",
    inner: "#8e241b",
    plate: "rgba(66,9,7,0.94)",
    glyph: "#ffe8bd",
    glow: "rgba(255,83,67,0.58)",
  },
  b: {
    rim: "#c8b081",
    inner: "#555a63",
    plate: "rgba(8,9,12,0.95)",
    glyph: "#eee4cf",
    glow: "rgba(196,183,153,0.42)",
  },
};
...
export function tacticalTokenTexture(kind: PieceKind, faction: Faction): THREE.CanvasTexture {
  ...
  drawPieceCharacter(ctx, kind, faction, 48);
  ctx.restore();
