...
/**
 * Web Audio mixer: wind/fire ambience, the licensed Chinese Guzheng 1 score
 * and a locally synthesised low war-drum tension stem.
 */
export class AudioManager {
  ...
  private async preload(): Promise<void> {
    const entries = Object.entries(AUDIO_URLS).filter(
      ([key, url]) => url.length > 0 && key !== "tension",
    );
    await Promise.all(
    ...
    // The war-drum layer stays synthetic so it can crossfade beneath the
    // licensed guzheng recording without adding another sampled dependency.
    this.buffers.set("tension", this.createWarDrumStem());
  }

  /** Eight-second low dagu/gong pulse used only when the position turns tense. */
  private createWarDrumStem(): AudioBuffer {
    const ctx = this.ctx;
    ...
    const strikes = [0, 2, 3.5, 5.5, 7];
    for (const start of strikes) {
      const offset = Math.floor(start * sampleRate);
      const length = Math.min(data.length - offset, Math.floor(sampleRate * 0.9));
      for (let i = 0; i < length; i += 1) {
        const t = i / sampleRate;
        const envelope = Math.exp(-t * 5.2);
        const pitch = 72 - t * 25;
        const skin = Math.sin(Math.PI * 2 * pitch * t) * 0.7 + Math.sin(i * 0.47) * Math.exp(-t * 22) * 0.22;
        data[offset + i] += skin * envelope * 0.38;
      }
    }
    for (let i = 0; i < data.length; i += 1) {
      const t = i / sampleRate;
      data[i] += Math.sin(Math.PI * 2 * 108.5 * t) * Math.exp(-((t - 4) * (t - 4)) / 3.8) * 0.025;
  ...
  private async primeDeathCries(): Promise<void> {
    ...
    const kinds: PieceKind[] = ["k", "q", "b", "n", "r", "c", "p"];
