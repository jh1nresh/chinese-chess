...
/** Vertical Chu/Han field standard, painted locally so it works offline. */
function battleStandardTexture(color: number, character: "楚" | "漢"): THREE.CanvasTexture {
  ...
  const glow = base.clone().lerp(new THREE.Color(0xffffff), 0.2);
  const gradient = ctx.createLinearGradient(0, 0, 256, 384);
  gradient.addColorStop(0, dark.getStyle());
  gradient.addColorStop(0.42, base.getStyle());
  gradient.addColorStop(1, dark.getStyle());
  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, 256, 384);

  ctx.strokeStyle = "#d5aa55";
  ctx.lineWidth = 12;
  ctx.strokeRect(18, 18, 220, 348);
  ctx.strokeStyle = glow.getStyle();
  ctx.lineWidth = 3;
  ctx.strokeRect(30, 30, 196, 324);

  ctx.fillStyle = "rgba(0,0,0,0.18)";
  for (let y = 42; y < 360; y += 34) ctx.fillRect(32, y, 192, 2);

  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.font = "700 138px KaiTi, STKaiti, DFKai-SB, serif";
  ctx.fillStyle = "rgba(20,8,5,0.66)";
  ctx.fillText(character, 132, 197);
  ctx.fillStyle = "#f4d995";
  ctx.fillText(character, 126, 190);
...
export class Battlefield {
  ...
  private buildCamps(): void {
    ...
    const sides: { sign: number; cloth: THREE.CanvasTexture; tint: number; banner: number; character: "楚" | "漢" }[] = [
      { sign: 1, cloth: this.track(clothTexture("#343737", "#24313b")), tint: 0x9d9a90, banner: 0x252b2d, character: "楚" },
      { sign: -1, cloth: this.track(clothTexture("#49302a", "#8a2420")), tint: 0x99857a, banner: 0x98231e, character: "漢" },
    ];
    ...
    for (const side of sides) {
      ...
      for (let i = 0; i < 9; i += 1) {
        ...
        if (i % 3 === 0) this.addBanner(x + side.sign * 2.6, z, side.banner, side.character);
      }
  ...
  private addBanner(x: number, z: number, color: number, character: "楚" | "漢"): void {
    ...
    const material = this.track(
      new THREE.MeshStandardMaterial({
        map: this.track(battleStandardTexture(color, character)),
        color: 0xffffff,
        roughness: 0.92,
        metalness: 0,
        side: THREE.DoubleSide,
