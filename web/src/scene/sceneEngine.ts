...
const CAMERA_SHOTS: Record<CameraPreset, CameraShot> = {
  white: { position: new THREE.Vector3(0, 7.2, 10.8), target: new THREE.Vector3(0, 0.35, 0) },
  black: { position: new THREE.Vector3(0, 7.2, -10.8), target: new THREE.Vector3(0, 0.35, 0) },
  top: { position: new THREE.Vector3(0, 15.2, 0.35), target: new THREE.Vector3(0, 0.2, 0) },
  cinematic: { position: new THREE.Vector3(10.8, 4.2, 8.6), target: new THREE.Vector3(-0.4, 0.5, -0.4) },
};
...
const SHOWCASE_SHOT: CameraShot = {
  position: new THREE.Vector3(8.4, 7.3, 10.2),
...
const GAITS: Record<PieceKind, Gait> = {
  ...
  c: { stepsPerTile: 1.9, cadence: 2.35, timbre: "leather", volume: 0.82 },
};
 ...
 * has burned away.
 */
const RANGED_KINDS: PieceKind[] = ["c"];
  ...
  private escalateFallback(): void {
    ...
    switch (this.fallbackStage) {
      case 1:
        ...
        this.report("此顯示裝置無法完成後製，已關閉電影特效。", false);
        return;
      case 2:
        ...
        this.applyEnvironment();
        this.report("此顯示裝置無法處理環境反射，已切換為基本天光。", false);
        return;
      default:
        if (this.safeMode) {
        ...
        this.report("顯示裝置無法繪製完整場景，已切換至安全顯示模式。", true);
        return;
    }
  }
